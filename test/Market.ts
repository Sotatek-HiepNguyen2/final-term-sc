...
describe("Market", function () {
  ...
  describe("Functions", async function () {
    ...
    describe("Trade", function () {
      let bannedUser: HardhatEthersSigner;
      ...
      describe("Direct sale validation", function () {
        it("Should revert if blacklisted", async function () {
          ...
          await expect(this.market.connect(bannedUser).buyItem(0)).to.be.revertedWithCustomError(
            this.market,
            "UserBanned",
          );
        });
      ...
      describe("Auction validation", function () {
        it("Should revert place new bid if blacklisted", async function () {
          ...
          await expect(
            this.market.connect(bannedUser).placeNewBid(0, 0, { value: BigInt(1e18) }),
          ).to.be.revertedWithCustomError(this.market, "UserBanned");
        });
      ...
      describe("Place a new bid", function () {
        ...
        describe("ETH bid", function () {
          ...
          it("Should revert place new bid if the auction is already ended", async function () {
            ...
            await expect(
              this.market.connect(this.bidder).placeNewBid(0, 0, { value: BigInt(3e18) }),
            ).to.be.revertedWith("Auction was ended");
          });
          ...
          it("Should revert if the bid ETH price is less than the current bid price plus bid increment", async function () {
            ...
            await expect(
              this.market.connect(this.bidder).placeNewBid(0, 0, { value: BigInt(2e18) }),
            ).to.be.revertedWith("New bid price need to greater than minimum price");
          });
    ...
    describe("Cancel auction", function () {
      ...
      it("Should revert if auctionId is invalid", async function () {
        await expect(this.market.connect(this.auctionCreator).cancelAuction(1)).to.be.revertedWith("Auction not exist");
      });
      ...
      it("Should revert if the auction is already ended", async function () {
        ...
        await expect(this.market.connect(this.auctionCreator).cancelAuction(0)).to.be.revertedWith("Auction was ended");
      });
