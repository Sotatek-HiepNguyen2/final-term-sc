...
const ACTUAL_AMOUNT_USER_HAS_TO_PAY = getActualAmountUserHasToPay(BUY_PRICE, INITIAL_BUY_TAX, TAX_BASE);
const SELLER_PROCEEDS = getSellerProceeds(BUY_PRICE, INITIAL_SELL_TAX, TAX_BASE);
  ...
  describe("Functions", async function () {
    ...
    describe("Setup tax fee", async function () {
      ...
      it("Should revert if sell tax fee is invalid", async function () {
        await expect(this.market.setTaxFee(101, 5)).to.be.revertedWithCustomError(this.market, "InvalidSellTax");
      });

      it("Should revert if buy tax fee is invalid", async function () {
        await expect(this.market.setTaxFee(5, 101)).to.be.revertedWithCustomError(this.market, "InvalidBuyTax");
      });
    ...
    describe("Trade", function () {
      ...
      describe("Direct sale validation", function () {
        ...
        it("Should revert if the item is already sold", async function () {
          ...
          await this.market
            .connect(this.seller)
            .listForSale(ethers.ZeroAddress, await this.erc721Token.getAddress(), ERC721_TOKEN_ID, 0, BUY_PRICE);
          await this.market.connect(this.buyer).buyItem(0, { value: ACTUAL_AMOUNT_USER_HAS_TO_PAY });
      ...
      describe("Listing and Buy item", function () {
        ...
        it("Should update the right amount of ETH procceeds", async function () {
          ...
          expect(sellerBalanceAfter - sellerBalanceBefore).to.equal(SELLER_PROCEEDS);
        });
      });
      ...
      describe("Cancel listing", function () {
        ...
        it("Should revert if saleId is invalid", async function () {
          await expect(this.market.connect(this.seller).cancelListing(1)).to.be.revertedWithCustomError(
            this.market,
            "SaleNotExists",
          );
        });
        ...
        it("Should revert if not the seller", async function () {
          await expect(this.market.connect(this.buyer).cancelListing(0)).to.be.revertedWithCustomError(
            this.market,
            "SellerOnly",
          );
        });
        ...
        it("Should revert if the item is already sold", async function () {
          ...
          await expect(this.market.connect(this.seller).cancelListing(0)).to.be.revertedWithCustomError(
            this.market,
            "ItemAlreadySold",
          );
        });
        ...
        it("Should send the erc1155 nft back to the seller", async function () {
          ...
          expect(await this.erc1155Token.balanceOf(this.seller.address, ERC1155_TOKEN_ID)).to.equal(ERC1155_QUANTITY);
        });
      ...
      describe("Place a new bid", function () {
        it("Should revert if auction not exist", async function () {
          await expect(this.market.connect(this.bidder).placeNewBid(0, 0, { value: 0 })).to.be.revertedWithCustomError(
            this.market,
            "AuctionNotExists",
          );
        });
        ...
        describe("ETH bid", function () {
          ...
          it("Should revert place new bid if the auction is already ended", async function () {
            await ethers.provider.send("evm_increaseTime", [AUCTION_DURATION + TIME_LEFT_TO_START_AUCTION]);
            await ethers.provider.send("evm_mine");

            await expect(
              this.market.connect(this.bidder).placeNewBid(0, 0, { value: VALID_BID_PRICE }),
          ...
          it("Should revert if the bid ETH price is less than the current bid price plus bid increment", async function () {
            const bidPrice = getActualAmountUserHasToPay(BigInt(2e18), this.buyTax, this.taxBase);
            await this.market.connect(this.bidder).placeNewBid(0, 0, { value: bidPrice });
            await expect(this.market.connect(this.bidder).placeNewBid(0, 0, { value: bidPrice })).to.be.revertedWith(
              "PlaceBid: New bid price need to greater than minimum price",
            );
          });

          it("Should place a new bid with ETH", async function () {
            const actualPrice = getActualAmountUserHasToPay(VALID_BID_PRICE, this.buyTax, this.taxBase);
            await this.market.connect(this.bidder).placeNewBid(0, 0, { value: actualPrice });
      ...
      describe("ERC20 bid", function () {
        ...
        it("Should revert if the bid ERC20 price is less than the current bid price plus bid increment", async function () {
          const bidPrice = getActualAmountUserHasToPay(BigInt(2e18), this.buyTax, this.taxBase);
          await this.erc20Token.connect(this.bidder).approve(await this.market.getAddress(), bidPrice);

          // Place first bid first
          await this.market.connect(this.bidder).placeNewBid(0, bidPrice);
          await expect(this.market.connect(this.bidder).placeNewBid(0, bidPrice)).to.be.revertedWith(
            "PlaceBid: New bid price need to greater than minimum price",
          );
        });
        it("Should place a new bid with ERC20", async function () {
          const actualPayAmount = getActualAmountUserHasToPay(VALID_BID_PRICE, this.buyTax, this.taxBase);
          await this.erc20Token.connect(this.bidder).approve(await this.market.getAddress(), actualPayAmount);
          await this.market.connect(this.bidder).placeNewBid(0, actualPayAmount);
      ...
      describe("Cancel auction", function () {
        ...
        it("Should revert if auctionId is invalid", async function () {
          await expect(this.market.connect(this.auctionCreator).cancelAuction(1)).to.be.revertedWithCustomError(
            this.market,
            "AuctionNotExists",
          );
        });
        ...
        it("Should revert if user has bid", async function () {
          await this.market.connect(this.bidder).placeNewBid(0, 0, { value: VALID_BID_PRICE });

          await expect(this.market.connect(this.auctionCreator).cancelAuction(0)).to.be.revertedWith(
            "CancelAuction: User already bidded",
          );
        });

        it("Should revert if not the auction creator", async function () {
          await expect(this.market.cancelAuction(0)).to.be.revertedWithCustomError(this.market, "NotAuctionCreator");
        });

        it("Should revert if the auction is already ended", async function () {
          await ethers.provider.send("evm_increaseTime", [TIME_LEFT_TO_START_AUCTION]);
          await ethers.provider.send("evm_mine");
      ...
      describe("End auction", function () {
        ...
        it("Should revert if auctionId is invalid", async function () {
          await expect(this.market.connect(this.auctionCreator).endAuction(1)).to.be.revertedWithCustomError(
            this.market,
            "AuctionNotExists",
          );
        });
        ...
        it("Should revert if the auction is not ended", async function () {
          await expect(this.market.connect(this.auctionCreator).endAuction(0)).to.be.revertedWith(
        ...
        it("Should end the auction", async function () {
          ...
          await ethers.provider.send("evm_increaseTime", [TIME_LEFT_TO_START_AUCTION + AUCTION_DURATION]);
          await ethers.provider.send("evm_mine");
        ...
        it("Should send nft back to the seller if no bid", async function () {
          ...
          expect(auction.isEnded).to.be.true;
          expect(await this.erc721Token.ownerOf(ERC721_AUCTION_TOKEN_ID)).to.equal(this.auctionCreator.address);
        });
        ...
        it("Should send nft to the highest bidder", async function () {
          ...
          const auction = await this.market.auctions(0);
          expect(auction.isEnded).to.be.true;
          expect(await this.erc721Token.ownerOf(ERC721_AUCTION_TOKEN_ID)).to.equal(this.bidder.address);
        });
        ...
        it("Should send the right amount of ERC20 to the seller", async function () {
          ...
          const acutualPrice = getActualAmountUserHasToPay(bidPrice, this.buyTax, this.taxBase);

          // Approve erc20
          await this.erc20Token.connect(this.bidder).approve(await this.market.getAddress(), acutualPrice);
          ...
          expect(await this.erc1155Token.balanceOf(this.bidder.address, ERC1155_TOKEN_ID)).to.equal(ERC1155_QUANTITY);
        });
    ...
    describe("Withdrawals", function () {
      ...
      it("Should withdraw the right amount of ETH", async function () {
        ...
        const bidPrice = VALID_BID_PRICE;
        const acutualPrice = getActualAmountUserHasToPay(bidPrice, this.buyTax, this.taxBase);
        await this.market.connect(this.bidder).placeNewBid(0, 0, { value: acutualPrice });
        ...
        await this.market.connect(this.auctionCreator).withdraw([ethers.ZeroAddress]);
        const sellerBalanceAfter = await this.market.getProceeds(
          await this.auctionCreator.getAddress(),
          ethers.ZeroAddress,
        );
      ...
      it("Should withdraw the right amount of ERC20", async function () {
        ...
        const acutualPrice = getActualAmountUserHasToPay(VALID_BID_PRICE, this.buyTax, this.taxBase);
        ...
        await this.market.connect(this.auctionCreator).withdraw([this.erc20Token.getAddress()]);
        const sellerBalanceAfter = await this.market.getProceeds(
          await this.auctionCreator.getAddress(),
          await this.erc20Token.getAddress(),
        );

        expect(sellerBalanceBefore - sellerBalanceAfter).to.equal(SELLER_PROCEEDS);
      });
    });
  });
});
